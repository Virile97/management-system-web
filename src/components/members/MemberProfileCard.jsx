...
import { Mail, Phone, CalendarDays, Droplet, Briefcase, MapPin, Pencil } from "lucide-react"
  ...
  return (
    <div className="flex items-start gap-3 border-b border-border px-4 py-3 last:border-0">
      ...
      <div className="min-w-0">
        ...
        <p className="text-sm wrap-break-word text-foreground/85">{value || "—"}</p>
      </div>
    </div>
  )
}
  ...
  return (
    <Card className="overflow-hidden rounded-2xl p-0">
      ...
      <div className="bg-white">
        ...
        <DetailRow icon={CalendarDays} label="Joined" value={formatDate(member.joinedAt || member.createdAt)} />
        <DetailRow icon={Droplet} label="Baptized" value={member.baptized} />
        <DetailRow icon={Briefcase} label="Occupation" value={member.occupation} />
