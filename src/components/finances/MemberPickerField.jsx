...
function MemberSearchModal({
  ...
  emptyHint = "Start typing to find a member.",
  suggestions = [],
  suggestionsLabel = "Suggested",
  noResultsHint = "No members found",
  searchFn = null,
}) {
  ...
  useEffect(() => {
    ...
    async function search() {
      ...
      try {
        const data = useCustomSearch
          ? await searchFn(debouncedQuery, controller.signal)
          : (
              await listMembers(
  ...
  }, [open, debouncedQuery, searchFn, useCustomSearch])
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent
        className="z-[60] flex max-h-[min(36rem,calc(100vh-4rem))] max-w-lg flex-col gap-0 overflow-hidden border border-border bg-card p-0 text-card-foreground shadow-lg sm:max-w-xl"
        showCloseButton={false}
      >
        <DialogHeader className="flex-row items-center justify-between gap-0 border-b border-border bg-card px-4 py-4 sm:px-5">
          <DialogTitle className="font-heading text-lg font-normal text-foreground/85">
            {title}
          </DialogTitle>
          <button
            type="button"
        ...
        <div className="min-h-[12rem] flex-1 overflow-y-auto bg-card">
          {!debouncedQuery && !showSuggestions ? (
            ...
            <>
              {showSuggestions ? (
                <p className="px-4 pt-3 pb-1 text-[11px] font-medium tracking-wide text-muted-foreground uppercase sm:px-5">
                  {suggestionsLabel}
                </p>
              ) : null}
              <ul>
