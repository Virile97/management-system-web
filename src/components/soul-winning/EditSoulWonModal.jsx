...
import { enqueueUpdateSoulWinningRecord } from "@/stores/processQueue.store"
...
function recordToForm(record) {
  ...
  return {
    soulWinners: (record?.soulWinners || []).map((winner) => ({
      id: winner.id,
      name: winner.name,
    })),
    wonAt: toDateInputValue(record?.date) || toDateInputValue(),
    firstName: convert.firstName || "",
    middleName: convert.middleName || "",
...
function EditSoulWonModal({ open, onOpenChange, record, onUpdated }) {
  ...
  function handleSubmit() {
    ...
    const winnersLabel = form.soulWinners
      ...
      .filter(Boolean)
      .join(", ")
    const convertName = [form.firstName, form.lastName]
      .filter(Boolean)
      .join(" ")
      .trim()

    // Queue so the modal closes immediately, same flow as
    // RecordSoulWonModal's queued create — the row stays as-is until the
    // update drains, then the table is patched from the optimistic snapshot
    // passed via onUpdated instead of waiting on a refetch.
    enqueueUpdateSoulWinningRecord({
      id: record.id,
      payload,
      label: convertName || "Soul winning record",
    })
    addAddress(form.location)

    onUpdated?.({
      ...record,
      date: form.wonAt || record.date,
      convert: {
        ...(record.convert || {}),
        firstName: form.firstName.trim(),
        middleName: form.middleName.trim() || null,
      ...
      convertName: convertName || record.convertName,
      contact: payload.contact,
      age: payload.age,
      location: payload.location,
      event: payload.event,
      notes: payload.notes,
      soulWinners: form.soulWinners,
      soulWinner: form.soulWinners[0] || null,
      soulWinnerName: winnersLabel || record.soulWinnerName,
      soulWinnerNames: form.soulWinners.map((w) => w.name).filter(Boolean),
      soulWinnerId: form.soulWinners[0]?.id || null,
    })
    onOpenChange(false)
  }
            ...
            <>
              <div className="flex flex-col gap-5 overflow-y-auto px-4 py-6 sm:px-6">
                ...
                <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                  <div className="flex flex-col gap-1.5">
                    <Label htmlFor="edit-soul-date">Date</Label>
                    <MemberDatePicker
                      id="edit-soul-date"
                      value={form.wonAt}
                ...
                <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
                  <div className="flex flex-col gap-1.5">
                    <Label htmlFor="edit-convert-first">
                      First name <span className="text-red-500">*</span>
                    </Label>
                    <Input
                      id="edit-convert-first"
                  ...
                  <div className="flex flex-col gap-1.5">
                    <Label htmlFor="edit-convert-middle">Middle name</Label>
                    <Input
                      id="edit-convert-middle"
                      className="h-10 rounded-lg"
                  ...
                  <div className="flex flex-col gap-1.5">
                    <Label htmlFor="edit-convert-last">
                      Last name <span className="text-red-500">*</span>
                    </Label>
                    <Input
                      id="edit-convert-last"
                      className="h-10 rounded-lg"
                      value={form.lastName}
                      onChange={(event) =>
                ...
                <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                  <div className="flex flex-col gap-1.5">
                    <Label htmlFor="edit-convert-age">Age</Label>
                    <Input
                      id="edit-convert-age"
                      type="number"
                      min={0}
                  ...
                  <div className="flex flex-col gap-1.5">
                    <Label htmlFor="edit-convert-phone">Contact</Label>
                    <Input
                      id="edit-convert-phone"
                      className="h-10 rounded-lg"
                      value={form.contact}
                      onChange={(event) =>
                ...
                <div className="flex flex-col gap-1.5">
                  <Label htmlFor="edit-convert-address">Address / Location</Label>
                  <SearchableCombobox
                    id="edit-convert-address"
                ...
                <div className="flex flex-col gap-1.5">
                  <Label htmlFor="edit-convert-notes">Notes</Label>
                  <textarea
                    id="edit-convert-notes"
                    rows={3}
                    value={form.notes}
...
export { EditSoulWonModal }
export default EditSoulWonModal
